...
├── stocks_universe.py            # KR Top50 + US S&P500 Top50 + ASIA_TOP 174종목 백필·dim 시드 (--seed-dim, 130→174 확장: 아시아종목.xlsx 미수집분 보강)
└── valuation.py                  # KOSPI PER/PBR/배당수익률

scripts/
├── auto_market.py                    # 일일 자동화 (토 18:50 + 화~금 06:50 KST, 한국 공휴일 반영; 월·일 스킵)
├── collect_weekly.py                 # 주간 수집 러너 (토 19:30 KST)
├── collect_securities_reports.py     # 미래에셋증권 상세분석 보고서 → S3
├── collect_prism_reports.py          # MVP PRISM 보고서 → S3 (증분 스캔)
├── generate_ocr_story.py             # 일일 브리핑 PDF → 1차 자료 보존 HTML (2026-07-25 자동화 제거, 수동 실행만)
...
- `.claude/commands/` — `/market-data`, `/market-deploy`, `/market-full`, `/market-full-b`, `/market-pm`, `/market-cs`, `/research`, `/review-story`, `/asia-weekly`
