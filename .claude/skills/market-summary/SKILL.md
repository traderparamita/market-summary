...
2. `<div id="tab-cs" class="tab-panel">` ~ `</div><!-- /tab-cs -->` 블록을 Edit 도구로 교체
3. 같은 내용으로 `output/summary/YYYY-MM/YYYY-MM-DD_cs.html` Edit (두 파일 동기화)

**주간/월간**: 같은 패턴 (`weekly/YYYY-WNN.html` + `_cs.html`, `monthly/YYYY-MM.html` + `_cs.html`).
