...
**표기 규칙 (한글·영문 병행)**:
- 지수·종목·통화명은 한글(영문) 형식: `코스피(KOSPI)`, `나스닥(NASDAQ)`, `금(Gold)`, `달러/원(USD/KRW)`, `WTI유(WTI)`
- 영문 약어도 한국어 병행: `사상 최고가(ATH)`, `연초 대비(YTD)`, `주간 대비(WTD)`, `월간 대비(MTD)`, `주가수익비율(PER)`, `변동성지수(VIX)`, `달러인덱스(DXY)`, `장단기 스프레드(10-2 Spread)`
...
- 지수·종목·통화명은 한글(영문) 형식: `코스피(KOSPI)`, `나스닥(NASDAQ)`, `금(Gold)`, `달러/원(USD/KRW)`, `WTI유(WTI)`
- 영문 약어도 한국어 병행: `사상 최고가(ATH)`, `연초 대비(YTD)`, `주간 대비(WTD)`, `월간 대비(MTD)`, `주가수익비율(PER)`, `변동성지수(VIX)`, `달러인덱스(DXY)`, `장단기 스프레드(10-2 Spread)`
- 색상 컨벤션: 상승=빨간(`pm-up`/`hl-up`), 하락=파란(`pm-dn`/`hl-down`) — 한국 주식창 기준

**포함 권장**:
- 종가 + 일변동률 (`코스피(KOSPI) 6,417.93 (+0.46%)`)
- 주간/월간/연초 대비(YTD) 누적 (`주간 대비(WTD) +2.5%, 월간 대비(MTD) +3.2%, 연초 대비(YTD) +8.1%`)
- 스프레드·커브 (`장단기 스프레드(10-2 Spread) +48bp`)
- FX 절대 레벨 + 변동 (`달러/원(USD/KRW) 1,392 (+0.3%)`)
- 주도주별 등락률 top 1~2개
