...
각 국가 단락의 닫는 `</p>` **직전**에 `<br><br>` 을 삽입해 시각적 여백을 확보한다 (Market Story Hero의 아시아/유럽/미국 세션 간 간격과 동일 패턴).

```html
<p><strong>🇰🇷 한국</strong>: ... 마지막 문장.<br><br></p>
<p><strong>🇺🇸 미국</strong>: ... 마지막 문장.<br><br></p>
<p><strong>🇯🇵 일본</strong>: ... 마지막 문장.<br><br></p>
...
```

마지막 단락(예: 미국 또는 보강 신흥아시아)에도 `<br><br>` 을 두면 Hero 카드 하단과 4섹션 표 사이에도 여백이 생겨 가독성이 향상됨.

**금지**: `<p style="margin-bottom:...">` 같은 inline style 사용 (story-text의 라인하이트 일관성 깨짐). `<br><br>` 만 사용.
