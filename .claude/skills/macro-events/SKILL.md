...
- ★: 주택지표, 소규모 지역 지표
- **구조적·기업 이벤트**(대형 IPO·공급과잉·산업정책·대형 단일종목 쇼크 등): 거시 캘린더엔 안 뜨지만 시장 파급이 크면 ★★~★★★로 **이벤트 테이블 또는 '구조적 이슈' 노트에 반드시 포함**. Step 2.5(카탈리스트 로그·전주 변수 회수)로 회수한다. FRED/ECOS 지표 검색만으로는 잡히지 않으므로 별도 확인이 필수.
...
### Step 2.5 — 카탈리스트 로그 소비 + 전주 변수 회수 (누락 방지) 🚨

거시지표 검색(Step 3~4)만으로는 **개별기업 IPO·산업 공급 이벤트·대형 단일종목 쇼크**가 통째로 누락된다.
(실제 사례: 2026-W30에서 中 CXMT 상하이 상장發 메모리 공급과잉 논쟁이 macro 탭에서 완전 누락 — 직전 W29가 'W30 최대 변수 ②·★★★'로 명시 예고했는데도.)
아래 (A)·(B)를 **반드시** 교차 확인한다.

**(A) 카탈리스트 로그 소비** — 이번 기간 + 직전 1~2주 촉매를 회수:

```bash
tail -n 60 output/catalysts/completed.jsonl
# 필요시 기간 grep: grep -E '"date": "2026-07-(1[3-9]|2[0-4])"' output/catalysts/completed.jsonl
```

각 줄 `name`에 **IPO·공급과잉·M&A·국가 산업정책·대형 단일종목 급락**이 있으면, 매크로 캘린더에 안 떠도
이벤트 테이블 또는 '🧩 구조적 이슈' 노트로 반영한다. (기간 밖 원인 사건은 날짜 행이 아니라 "배경(직전 주)" 라벨의 노트로 — 리뷰/캘린더 분리 원칙 준수.)

**(B) 전주 '다음 기간 최대 변수' 회수** — 직전 기간 `_macro.html`을 열어 "최대 변수" 불릿과 ★★★ 캘린더 항목을 확인:

```bash
grep -nA3 "최대 변수" output/summary/weekly/{직전기간}_macro.html   # 월간이면 monthly/
```

각 항목이 이번 리뷰에서 **① 실제 발생 → 리뷰(또는 구조적 노트) 반영, 또는 ② 지속 → 이번 캘린더로 이월** 중 하나로 처리됐는지 대조한다. 어느 쪽도 아니면 누락이므로 채운다.
...
- [ ] **카탈리스트 로그(`completed.jsonl`)를 이번+직전주 스캔**해 IPO·공급과잉·산업정책 등 비(非)거시 촉매를 회수했는가? (Step 2.5-A)
- [ ] **직전 기간이 예고한 '최대 변수'·★★★ 항목을 회수**해 리뷰 반영 또는 캘린더 이월했는가? (Step 2.5-B)
- [ ] 기간 밖(직전 주) 원인 사건을 날짜 이벤트 행이 아니라 "배경" 노트로 분리했는가? (리뷰/캘린더 분리)
