...
### Step 5-C: 주간 PM Story 작성 (마지막 영업일만)

Step 5 완료 후 (Step 5-B 와 독립) `market-summary` 스킬의 **"PM Story 작성 절차" — 주간/월간 PM Story 적용 가이드** 섹션 규칙으로 주간 PM 버전을 작성한다.

핵심:
1. `output/summary/weekly/YYYY-WNN_story.html` + 일간 5개 `_data.json` (해당 주) Read
2. 6 고정 섹션(🇰🇷 한국 · 🌐 매크로 · 🌏 아시아 및 중국 · 🇺🇸 미국 · 🇪🇺 유럽 · 💵 채권)
3. 각 섹션 4~6 불릿. WTD 수익률 + 주중 최대 상승·하락일 + 핵심 이벤트
...
### Step 7-C: 월간 PM Story 작성 (마지막 영업일만)

Step 7 완료 후 (Step 7-B 와 독립) `market-summary` 스킬의 **"PM Story 작성 절차" — 주간/월간 PM Story 적용 가이드** 규칙으로 월간 PM 버전을 작성한다.

핵심:
1. `output/summary/monthly/YYYY-MM_story.html` + 일간 `_data.json` (해당 월) Read
2. 6 고정 섹션. 각 섹션 5~7 불릿. 월말 종가 + MTD + YTD + 월간 최대 상승·하락일 + 주차별 리듬 + 주요 이벤트
3. `monthly/YYYY-MM.html` 의 `<div id="tab-pm">` 블록 Edit + `_pm.html` sibling 동기화
